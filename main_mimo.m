...
Ntrans = 4; % For MIMO
Nrecv = 4;
Nmin = min([Ntrans Nrecv]);

Power = [27+12 36+6 27+30 22+60];%[17+12 20+12 27+12 36+6];% [50mW 100mW 500mW 1W] PtGtGr = 36+6;% dBm % 10^((36+6)/10); % linear scale

% Parameters of path loss model, page 41, Mark
fc = 2.4e3; %MHz
height = [2 9 15 24 30 45];%[2 9 15 21 24 30 45]; % use a set instead of a fixed value h = 9; %m
sigma = 8; % Shadowing standard deviation
sigma_awgn_exp = 18; % In fact it is 21
...
M_hybrid = 2; % Hybrid chain parameter, positions of redundant peer every mth node
...
%         end
        ...
        Prob = 0.5*(1 - erf((10*log10((2^(R/Nmin) - 1)*sigma_awgn*Nmin/Nrecv) - alpha + beta.*log10([1:Distance]))/(sqrt(2)*sigma))); % Ps = 1 - P_outage
        flag_mimo = 1;
        ...
        filename = strcat('results_n_M',num2str(M_hybrid),'_TR',num2str(T_R),'_',num2str(sigma_awgn_exp),'_h',num2str(h),'_P',num2str(PtGtGr));
        save(filename);
        done = 10
    end
end
