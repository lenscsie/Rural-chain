...
if isempty(I) ~= 1
        ...
        for j = 1:Ntrials
            ...
            while(nreached+ndropped < Npackets)
                ...
                tmp_f = rand(1,length(state)) <= (1 - fr(state));
                tmp_f_r = rand(1,length(state_r)) <= (1 - fr(state_r));
                state = tmp_f + 1;
                state_r = tmp_f_r + 1;
        ...
        capacity_hybrid_m(i,height_index,Power_index) = mean(capacity_tmp*packet_size/step_time*1e-6); % in Mbits/s
        capacity_hybrid_m_std(i,height_index,Power_index) = std(capacity_tmp*packet_size/step_time*1e-6); % in Mbits/s
        P_emp_hybrid_m(i,height_index,Power_index) = mean(ndelivered/Npackets);
        P_emp_hybrid_m_std(i,height_index,Power_index) = std(ndelivered/Npackets);
    end
end
