...
for i = N_max:-1:1%1:length(d);
    ...
    if isempty(I) ~= 1
        ...
        capacity_etx_m(i,height_index,Power_index) = mean(capacity_tmp*packet_size/step_time*1e-6); % in Mbits/s
        capacity_etx_m_std(i,height_index,Power_index) = std(capacity_tmp*packet_size/step_time*1e-6);
        P_emp_etx_m(i,height_index,Power_index) = mean(ndelivered/Npackets);%ndelivered/Ntrials/Npackets;
        P_emp_etx_m_std(i,height_index,Power_index) = std(ndelivered/Npackets);
    end
end